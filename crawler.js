...
let depth; // size of a batch - can be set via command line
      ...
      crawlSinglePage(uri, depth)
        .then(singlePageData => {
          ...
          if(!crawledUrls.includes(singlePageData.uri)) {
            ...
            if(singlePageData.assets.length > 0) {
              output.push({
                sourceUrl: singlePageData.uri,
                imageUrl: singlePageData.assets,
                depth: depth
              });
            }
          }
...
function init() {
  ...
  // parses depth size from command line
  depth = argv.depth || 3;
