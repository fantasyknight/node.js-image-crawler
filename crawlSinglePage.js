...
function extractInternalHyperlinks(linksCollection, baseUrl, depth) {
  ...
  let filteredLinksCollection = linksCollection.filter(linkObject => {
    if(baseUrl && linkObject.attribs.href) {
      ...
      const objectPath = linkObject.attribs.href;
      const linkDepth = objectPath.slice(-1) === '/' ? objectPath.split('/').length - 2 : objectPath.split('/').length - 1;
      ...
      return (linkDomain === hostname) && (linkDepth < depth + 1) && validProtocol && notAnImage;
    }
  });
...
function extractAssetUrls(elementsCollection, assets, baseUrl) {
  elementsCollection.map(elem => {
    if(elem.attribs.src || elem.attribs.href) {
      ...
      const onlyImage = (/\.(gif|jpe?g|tiff|png|pdf)$/i).test(assetLocation); // to filter out images and pdfs
      if(onlyImage) {
...
// }
function crawlSinglePage(pageUrl, depth) {
    ...
    .then(response => {
      ...
      if(response && isHTML && is200) {
        ...
        return {
          uri: pageUrl,
          assets: assets,
          internalHyperlinks: extractInternalHyperlinks(hyperlinks, baseUrl, depth) // filterning out external links
        };
      }
      return {
        uri: pageUrl,
